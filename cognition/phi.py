...
用法:
    from cognition.phi import UnifiedPhi, PhiModulator, ConsciousnessDetector
  phi = UnifiedPhi()
...
class UnifiedPhi:
    ...
    last_selected_alpha: float = 0.25
    ...
    @property
    def dmn_depth(self) -> float:
        """Forward-compatible alias for temporal_depth."""
        return self.temporal_depth

    @dmn_depth.setter
    def dmn_depth(self, value: float):
        self.temporal_depth = clamp(value)
        self._sources_valid["temporal_depth"] = self.source_ttl
    ...
    def feed_from_impact(self, impact: Optional[CognitiveImpactProfile]):
        """Route a cognitive impact profile into the active source model."""
        if impact is None:
            return

        self.sensory_integration = clamp(impact.sensory)
        self.temporal_depth = clamp(impact.cognitive)
        self.self_reflection = clamp(impact.self_)
        self.global_ignition = clamp(impact.novelty)
    ...
    def select_alpha(self, max_event_intensity: float) -> float:
        """Select EMA alpha adaptively from current event intensity."""
        intensity = clamp(max_event_intensity)
        if intensity > 0.60:
            return 0.55
        if intensity >= 0.30:
            return 0.30
    ...
    def _infer_max_event_intensity(self) -> float:
        """Use active source magnitudes as a local proxy for incoming event intensity."""
        active_values = []
        for key in self.weights:
            if self._sources_valid.get(key, 0.0) > 0:
                active_values.append(getattr(self, key))
    ...
    def aggregate(self, max_event_intensity: Optional[float] = None) -> float:
        """
        聚合所有子成分 → 统一 Φ 值
        ...
        if max_event_intensity is None:
            max_event_intensity = self._infer_max_event_intensity()

        self.last_selected_alpha = self.select_alpha(max_event_intensity)

        # 自适应指数平滑：强事件快速抬升，弱事件慢变化
        self._phi = smooth(self._phi, self._phi_raw, self.last_selected_alpha)
...
class AdaptiveAlphaICRI(UnifiedPhi):
    """ICRI-compatible consciousness engine with adaptive EMA aggregation."""
