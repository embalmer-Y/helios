...
sequenceDiagram
    ...
    Note over User,QQQ: QQ 入站异步发生, 先进入消息队列
...
阅读重点：外部输入先被标准化为 `ChannelMessage`；被动回复与主动行为是两条路径；当前稳定主出站仍然是 QQ。
