"""R-PROTO-LEARN.19 owner 16a outward_expression learner tests."""

import pytest

from helios_v2.learning import (
    OutwardExpressionLearner, OutwardExpressionLearnerConfig, Regime,
)


def test_r19_default_config_uses_p5_feel_calibration():
    c = OutwardExpressionLearnerConfig()
    assert c.learning_rate == 0.05
    assert c.commit_threshold == 0.3
    assert c.min_stable_ticks == 8


def test_r19_input_output_dims():
    c = OutwardExpressionLearnerConfig()
    assert c.input_dim == 7
    assert c.output_dim == 9  # 3 + 3 + 3 policies


def test_r19_update_returns_snapshot():
    learner = OutwardExpressionLearner()
...
def test_r19_update_with_dict_state():
    learner = OutwardExpressionLearner()
    state = {
        "draft_intensity": 0.7,
        ...
        "acetylcholine": 0.4,
        "novelty": 0.5,
        "last_publish_tick": 0.6,
    }
    snap = learner.update(state, (0.5,) * 7, novelty=0.5, tick_id=0)
    assert snap is not None


def test_r19_update_with_dataclass_state():
    from dataclasses import dataclass

    @dataclass
    class OutwardExpressionState:
        draft_intensity: float = 0.5
        governance_pressure: float = 0.5
        identity_strength: float = 0.5
        dopamine: float = 0.5
        acetylcholine: float = 0.5
        novelty: float = 0.5
        last_publish_tick: float = 0.5

    learner = OutwardExpressionLearner()
    state = OutwardExpressionState(draft_intensity=0.8, identity_strength=0.7)
    snap = learner.update(state, (0.5,) * 7, novelty=0.5, tick_id=0)
    assert snap is not None


def test_r19_update_with_none_state_uses_defaults():
    learner = OutwardExpressionLearner()
    snap = learner.update(state=None, llm_signal=(0.5,) * 7, novelty=0.5, tick_id=0)
    assert snap is not None


def test_r19_closed_loop_residual_in_unit_interval():
    learner = OutwardExpressionLearner()
...
def test_r19_weights_learn_or_stay_valid():
    learner = OutwardExpressionLearner()
...
def test_r19_regime_starts_exploratory():
    assert OutwardExpressionLearner().regime() == Regime.EXPLORATORY


def test_r19_commit_count_starts_zero():
    assert OutwardExpressionLearner().commit_count() == 0


def test_r19_rejects_invalid_novelty():
    learner = OutwardExpressionLearner()
...
def test_r19_rejects_wrong_llm_signal_dim():
    learner = OutwardExpressionLearner()
    with pytest.raises(ValueError):
        learner.update(None, (0.5,) * 5, novelty=0.5, tick_id=0)


def test_r19_rejects_out_of_range_llm_signal():
    learner = OutwardExpressionLearner()
    with pytest.raises(ValueError):
        learner.update(None, (0.5,) * 6 + (1.5,), novelty=0.5, tick_id=0)


def test_r19_regime_can_transition():
    learner = OutwardExpressionLearner()
    for i in range(30):
...
def test_r19_weights_snapshot_immutable():
    learner = OutwardExpressionLearner()
...
def test_r19_bias_snapshot_immutable():
    learner = OutwardExpressionLearner()
...
def test_r19_does_not_modify_canonical_state():
    learner = OutwardExpressionLearner()
    state = {
        "draft_intensity": 0.7, "governance_pressure": 0.5,
        "identity_strength": 0.6, "dopamine": 0.7, "acetylcholine": 0.4,
        "novelty": 0.5, "last_publish_tick": 0.6,
    }
