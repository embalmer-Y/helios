"""R93: focused tests for the additive `reply_text` field on
...
def test_evidence_default_reply_text_is_empty() -> None:
    ev = StructuredThoughtEvidence(**_kwargs())
    assert ev.reply_text is None


def test_evidence_accepts_explicit_reply_text() -> None:
    ev = StructuredThoughtEvidence(**_kwargs(reply_text="hello operator"))
    assert ev.reply_text == "hello operator"


def test_evidence_remains_frozen_after_r93_field_added() -> None:
    ev = StructuredThoughtEvidence(**_kwargs(reply_text="hi"))
    with pytest.raises(Exception):
        ev.reply_text = "mutated"  # type: ignore[misc]


def test_reply_text_constants_are_stable() -> None:
    """Cross-owner contract: composition / planner / future tests rely on the cap shape."""
    assert REPLY_TEXT_MAX_CHARS == 2000
    assert REPLY_TEXT_TRUNCATION_SUFFIX == "…(truncated)"


def test_evidence_default_keeps_pre_r93_shape_byte_for_byte() -> None:
    """A pre-R93 caller that never sets `reply_text` produces an evidence whose
