...
> 状态：活文档（前向开发规划）。最近同步：**R101 — 6 维客观重要性 + 双重确认 + P5 基础（2026-06-17）**。R101 为双轨记忆第二片：6 维 `ObjectiveImportanceVector`（刺激强度 / 皮质醇 / 唤醒 / 结局权重 / 新颖度 / 关系风险）+ `OUTCOME_CLASS_WEIGHTS` 8 类映射（脑模型平行：HPA 轴 / 皮层唤醒 / 结局归类）；`DoubleConfirmationGate` 4 类（both_pass / objective_only / subjective_only / skip），`OBJECTIVE_PASS_THRESHOLD=0.50` + `SUBJECTIVE_PASS_THRESHOLD=0.60`；`ObjectiveImportanceLayerResolver`（skip→L2，self_changed L2→L5，PROMOTE=0.70 / DEMOTE=0.85）；`RecallUtilityTracker`（RECALL_EMA_ALPHA=0.3 滑动平均）；`MemoryImportanceLoss` + `MiningRecord` + `SqlBackedTrainingDatasetExtractor`（6 SQL 过滤：min_recall_count / min_objective_score / layer_filter / double_confirmation_filter / since_wall_seconds / limit）。P5 友好加性存储：`MemoryRecord` 增 9 字段（`objective_importance` JSON 编码 6 维向量 + `objective_score` + `subjective_score` + `double_confirmation` 4 类 + `recall_count` + `last_recall_at_tick` + `recall_utility_score` EMA + `last_updated_at_wall` + `promotion_history` 列表）；`PersistedExperienceRecord` 增 8 字段（1 JSON 向量 + 4 indexed + 3 utility）；SQLite PRAGMA-guarded `ALTER TABLE` 增 8 列（可幂等）。`MemoryAffectReplayEngine` 增 6 注入点 + `r101_active` 分支（任一 seam 为 None → legacy R100 路径字节级）。`RuntimeProfile` 增 6 R101 纵缝 + all-or-nothing 验证（`__post_init__` 拒绝 partial set）。semantic 装配接 6 first-version 实现；default 装配全 None（legacy 字节级）。165 新增 R101 测试（9 文件：`test_r101_objective_importance_vector` + `..._aggregator` + `..._estimator` + `..._double_confirmation_gate` + `..._layer_resolver` + `..._loss_and_extractor` + `..._memory_engine_dual_confirmation` + `..._persistence_objective_importance` + `..._composition_wiring`）。先前同步 **R100 — MemoryRecord schema + 4 层时间分层（2026-06-17）**。R100 为双轨记忆第一片：新增 `MemoryLayer` Literal 4 分层 + `MemoryRecord` 冻型（含 `layer` / `affect_intensity_at_write` / `outcome_class_at_write` / `memory_metadata`）；`AffectOutcomeMemoryLayerClassifier` 在写入时刻从 `affect_intensity` + `outcome_class` 决定初始层；`PersistedExperienceRecord` 增 `layer: MemoryLayer | None = None` + `memory_metadata: Mapping[str, str]` 加性字段；SQLite PRAGMA-guarded `ALTER TABLE` 添加 `layer TEXT` + `memory_metadata TEXT` 列；`ExperienceStore.search_similar(preferred_layers=...)` 按 `layer_preference_weight=1.5` 提升 preferred 层命中；`read_recent(layer_filter=...)` 按层过滤；`ExperienceRecordBridge` 从 `MemoryRecord` 投射 `layer` / `memory_metadata` 到 `PersistedExperienceRecord`；`RuntimeProfile.memory_layer_classifier` + `memory_layer_preference` 纵缝；semantic 装配接 `AffectOutcomeMemoryLayerClassifier()` + `("L4_long", "L5_autobiographical")`；default 装配不变（字节级）。64 新增 R100 测试。全量回归 1329 passed / 4 skipped / 0 失败。先前同步 **R99 — W4 情感验收探针正式化（2026-06-17）**。R99 交付 tests-only 情感验收探针（`tests/r99_emotion_validation_probe/`），自包含 `run_emotion_validation_probe(config)` 驱动生产形 runtime + fake-LLM gateway，测四 bounded `[0,1]` metric：cortisol_valence_separation=0.0785（≥0.05）、thought_content_grounding=1.0、memory_recall_relevance（诚实）、reply_loop_closure=0.8。EmotionValidationReport 的 `report_usable` 升级 R89 `bio_responsiveness` 轴从 drift-reconstruction 到 R99 emotion-probe reconstruction（usable→available/reconstructed；absent/unusable→drift path 保留）。R89 additive 接口（`emotion_validation_probe` 参数 + `emotion_validation_probe_usable` 字段 + `_emotion_probe_or_drift_bio_axis` helper）。16 测试全绿 + R89/R90 基线不破。1172 passed / 4 skipped。先前同步 **R98 Scope Extension** 和 **R98 — Post-LLM Appraisal Adjustment（2026-06-16）**。
...
| **6** | **R101** | 6 维客观重要性 + 双重确认 + P5 基础（**已交付 2026-06-17**） | 6 维 `ObjectiveImportanceVector`（刺激强度 / 皮质醇 / 唤醒 / 结局权重 / 新颖度 / 关系风险）；`OUTCOME_CLASS_WEIGHTS` 8 类；`DoubleConfirmationGate` 4 类（both_pass / objective_only / subjective_only / skip）；`ObjectiveImportanceLayerResolver`（skip→L2；self_changed L2→L5；PROMOTE=0.70 / DEMOTE=0.85）；`RecallUtilityTracker`（RECALL_EMA_ALPHA=0.3）；`MemoryImportanceLoss` + `SqlBackedTrainingDatasetExtractor`（6 SQL 过滤）；`MemoryRecord` 增 9 字段；`PersistedExperienceRecord` 增 8 字段（1 JSON + 4 indexed + 3 utility）；SQLite PRAGMA-guarded `ALTER TABLE` 增 8 列；`MemoryAffectReplayEngine` 增 6 注入点 + `r101_active` 分支；`RuntimeProfile` 增 6 R101 纵缝 + all-or-nothing 验证；semantic 装配接 6 first-version 实现；default 装配全 None（legacy R100 路径字节级）。165 新增 R101 测试（9 文件）。全量回归：R100 1329 + R101 165 = ~1500+ passed / 0 failed。 | R100 | 低 |
| **7** | **R102+** | 双轨记忆后续切片（衰减 / 检索规模化 / 工具 / 治理） | Ebbinghaus 衰减 + recall 重固化 + 自动晋升；bounded-window/ANN；`memory_tool_channel`（`30` LLM 记忆工具 recall/forget/consolidate）；forget 治理 fail-closed | R101 | 低 |
...
| R101 | 双轨记忆第二片：6 维客观重要性 + 双重确认 + P5 基础 | 6 维 `ObjectiveImportanceVector`（刺激强度 / 皮质醇 / 唤醒 / 结局权重 / 新颖度 / 关系风险；脑模型平行 HPA 轴 / 皮层唤醒 / 结局归类）；`OUTCOME_CLASS_WEIGHTS` 8 类（self_changed=0.95 / world_changed=0.80 / continuity_written=0.70 / executed=0.55 / rejected=0.40 / blocked=0.35 / internal_only_decision=0.25 / no_outcome=0.20）；`DoubleConfirmationGate` 4 类（both_pass / objective_only / subjective_only / skip，阈值 0.50 / 0.60，PROMOTE=0.70 / DEMOTE=0.85）；`ObjectiveImportanceLayerResolver`（skip→L2；self_changed L2→L5）；`RecallUtilityTracker`（RECALL_EMA_ALPHA=0.3 滑动平均）；`MemoryImportanceLoss` Protocol + `MiningRecord` + `SqlBackedTrainingDatasetExtractor`（6 SQL 过滤：min_recall_count / min_objective_score / layer_filter / double_confirmation_filter / since_wall_seconds / limit）。P5 友好加性存储：`MemoryRecord` 增 9 字段（objective_importance / objective_score / subjective_score / double_confirmation / recall_count / last_recall_at_tick / recall_utility_score / last_updated_at_wall / promotion_history）；`PersistedExperienceRecord` 增 8 字段（1 JSON 向量 + 4 indexed + 3 utility）；SQLite PRAGMA-guarded `ALTER TABLE` 增 8 列（可幂等）。`MemoryAffectReplayEngine` 增 6 注入点（objective_importance_estimator / objective_aggregator / double_confirmation_gate / objective_layer_resolver / recall_utility_tracker / hormone_prediction_provider）+ `r101_active` 分支（任一 seam 为 None → legacy R100 路径字节级）。`RuntimeProfile` 增 6 R101 纵缝 + all-or-nothing 验证（`__post_init__` 拒绝 partial set with `CompositionError`）。semantic 装配接 6 first-version 实现；default 装配全 None。165 新增 R101 测试（9 文件）。全量回归：R100 1329 + R101 165 + 0 失败。 |
...
> 6. **R101** — 6 维客观重要性 + 双重确认 + P5 基础（双轨记忆第二片）✅ 已交付（2026-06-17）
> 7. **R102+** — 双轨记忆后续切片（衰减 / 检索规模化 / 工具 / 治理，详见 §5）
> 8. **并行轨**：P4 网络通道生态（QQ/飞书/语音，详见 §6）
>
