...
> 状态：活文档（前向开发规划）。最近同步：**R97 — 去英文中心 / 中文 Appraisal Grounding（B3 根因闭合，2026-06-15）**。R96 闭合 B2（hash embedding 无语义，1112 passed / 4 skipped）；R97 闭合 B3（R40 threat/reward 原型是英文，中文情绪输入 cosine ≈ 0）：新增多语 `appraisal.anchor_catalog.AnchorCatalog`，first-version `DEFAULT_ANCHOR_CATALOG` 是双语（5 中文 threat + 5 中文 reward + R40 英文 anchor 复用保持 R40 字节级），`GroundedDimensionEstimator` 增 `anchor_catalog` 字段（`field(default_factory=...)` 打破 import 环）取 `max(R40 原型, catalog 维度)`。17 个新测试（6 catalog + 8 estimator + 3 B3 closure）+ 6 份 B3 闭合报告写入 `helios_v2/logs/r97_b3_closure/`。`scripts/r96_b2_real_llm_probes/analyze.py` 加 `b3_closed_real_llm: bool | None`（B3 阈值 0.10 比 B2 0.05 更严）。1126 passed / 4 skipped / 5 预存在 wall-clock-profile + lt1 失败。R56/R57 owner 边界 + R95-followup no-adhoc-logging guard 通过。**R96 拍板**：A — 云端 OpenAI-compatible（与 `.env` 现有 `HELIOS_EMBEDDING_API_KEY` / `HELIOS_EMBEDDING_BASE_URL` 兼容；R34 `OpenAICompatibleEmbeddingProvider` 已就位）。**R97 拍板**：A+B 混合（先翻译锚点 ship + 后续 P5 评估数据驱动迭代）。
> **📌 下个迭代明确顺序（2026-06-15 主人拍板 + R97 实施后）**：W3 #1 **R96**（真实语义 embedding，OpenAI-compatible 云端，**已收口**）→ W3 #2 **R97**（中文 appraisal grounding，**已收口**）→ W4 **R98**（情感验收探针正式化）→ **R99+**（双轨记忆，建在 R96 真实 embedding 上）→ 并行：P4 网络通道生态（QQ/飞书/语音）。内心独白方案已被 R91 present-field 通道覆盖，不再单独立项。
