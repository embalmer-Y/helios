...
1. **Gate no-fire chain closure (surfaced by R53, now CLOSED by R54).** R53 makes high real compute load correctly drive the gate to `resource_pressure_too_high` / no-fire. At R53 the assembled chain had no no-fire path (directed retrieval and the fired-path stages raised on a non-fired gate), so a high-load tick could not complete end to end; R53 therefore exercised the real `workload_pressure` only within the firing window end to end and validated the high-load → block relationship at the owner-neutral helper level. **R54 closed this**: each post-gate stage now emits an explicit not-activated result on a no-fire gate and the tick closes through the existing internal-only continuity tail, so a high-load tick now completes end to end as a no-fire tick across the full load range.
