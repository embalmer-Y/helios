...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R94（W2.5：彻底移除 `i_want_to_say`；让 LLM 真正决定动作 + 通道——R93 P1 的 `i_want_to_say` envelope 字段已**完全删除**；回复文本迁移到 `reply_text: str | None`（`action_intent="reply"` 的子细节）；`_emit_proposal` 的兼容路径已删除；结构化测试 `test_internal_thought_no_i_want_to_say_in_prompt.py` 在 system prompt / parser / fixture 三个层面强制断言无遗留字面值；通道路由沿用 R93 P2 `bound_user_ids` + planner 优先级）。先前同步 R93 Phase 2（W2：action_intent / target_user_id / bound_user_ids / planner 优先级）+ R93（W2：i_want_to_say 回复环闭合）+ R92（W1 wall-clock）+ R91（W1 present-field）+ R90（P5 memory fidelity probe）+ R89（P5 图灵 harness）+ R88（P5 漂移评估器）+ R83（长跑稳定性 harness）。
