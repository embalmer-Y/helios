...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R61（`06` prediction-mismatch 去 shim：惊讶证据由真实 `03` novelty grounded,退休 `0.8` 常量;真实 novelty→巩固显著性 + autobiographical/episodic 家族;familiar percept→None→episodic;诚实标注 novelty-as-surprise 非预测编码误差）。测试基线：735 passed。版本：R61。文档澄清（R41 后）：16 外化执行标注为非授权的前运动预备草案。
