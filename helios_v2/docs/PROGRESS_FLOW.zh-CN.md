...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R90（P5 评估框架完成——R88/R89/R90 均为 `tests/` 下只读 tests-only 诊断，**无 owner/阶段链/边界变更**，故下方着色与 R87 一致：**R88** 行为漂移评估器（P5 启动门，按 R83 轨迹把每个 owner 维分类为 `drift_positive`/`drift_negative`/`drift_neutral`/`dim_unavailable`），**R89** 长跑图灵式 harness（§13.4 六锁定 rubric 轴、证据锚定、双相似维保守聚合；离线基线诚实 `incomplete`/不通过），**R90** 记忆保真探针（真实 R10+R15 端到端 `recall_hit_rate`/`writeback_persistence_rate`/`latency_score`，经一处 additive harness 参数替换 R89 `memory_fidelity` stub）。测试基线：996 passed / 4 skipped（离线）。先前同步 R87（Consequence-Truth 真实送达对账，B4 收口 → **P0–P3 达 100%**：`17` 把对 effector 动作的 consequence corroboration 从"流程完成"升级为**真实送达可证伪**。`ConsequenceClaim` 加 `decision_id`/`selected_op`/`op_effect_class`/`op_user_visible`；bundle 加 `delivered_tool_result_evidence`（composition 从同帧 `channel_inbound_drain` 投影本 tick 回流的 correlation decision_id+ok）；新 `_corroborate_delivery` 发 `really_delivered`/`delivered_failed`(+告警)/`delivery_unverified`(诚实缺席)/`delivery_not_applicable`。严格 additive：R32 verdict/taxonomy/打分不变；只读。`effect_class` 成为真实消费者。无 owner 新增、阶段链不变。测试基线：968 passed / 4 skipped（离线）。先前同步 R86（受治理 OS 命令执行 effector + `13` 强制 risk-class 门 + `14` 两-tick fail-closed governed-action 授权握手：新 `helios_v2.channel.drivers.os_command`（`run_command`，op 级 `risk_class="governed"`，argv-前缀 default-deny allowlist——`unrestricted` 只读/诊断 + `governed` `mkdir`/`cp`/`mv`），no-shell/sandbox cwd/超时，注入式 `CommandExecutor`+`CommandRunner`（CI 用 `FakeCommandRunner` 无子进程），`tool_result` 回流。`13` 把 R85 的 `risk_class` read-through 升级为强制 fail-closed 门；`14` additive 扩展为 governed 动作授权权威（两-tick carry）。解释器/写自身代码永久 restricted。测试基线：957 passed / 4 skipped。先前同步 R85（FG-4 自主工具闭环：`11` 认知选工具 → `12` 归一化 → `13` 绑定+校验 → 执行 → 结果回流 `02`；driver 自描述 per-op `ChannelOpSpec`；取消 `tool` scope；planner-rejection tick 写 `world_blocked`。测试基线：921 passed / 4 skipped。版本：R87。
