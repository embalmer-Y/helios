...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R60（`06` 记忆内容去 shim：binding-context 内容从真实 `02` percept 派生,退休 `("hello","novelty")` 常量;空 percept 绑定锚定真实 `05` 体感的 honest no-percept 标记;默认装配行为变更已记录）。测试基线：732 passed。版本：R60。文档澄清（R41 后）：16 外化执行标注为非授权的前运动预备草案。
...
flowchart TD
    ...
    S06["06 记忆情感与重放 - 形成已去shim+情感记忆耐久/语义召回+召回过往情感记忆作多候选(R52)+记忆内容由真实percept派生(R60)"]:::base
    S07["07 工作空间竞争 - 真实竞争(读真实05体感)+有界注意力瓶颈+真实多候选(R52,语义装配)"]:::base
    S08["08 可报告意识内容 - 真实点火承诺(语义装配);R52起对真实多候选点火赢家;上游06/07已去shim"]:::base
    S09["09 思考门控与延续压力 - NE arousal + 工作空间激活 + workload_pressure(R53) + temporal/DMN(R55)接真实/drive_urgency 仍shim"]:::base
