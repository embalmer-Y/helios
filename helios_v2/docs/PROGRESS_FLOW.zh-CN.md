...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R59（外部 afferent 诚实化：`RuntimeProfile.external_signal_source` 让真实外部 `SensorySource` 可注入,取代常量 placeholder;首版 `SequenceExternalSignalSource` 只回放调用方真实信号、绝不编造;默认 placeholder 标注为非真实;与 `channel_cli` 互斥;opt-in、default-off）。测试基线：728 passed。版本：R59。文档澄清（R41 后）：16 外化执行标注为非授权的前运动预备草案。
