...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R79-A（激进-激进-反戏剧化 prompt 路径落代码：`AggressiveRadicalEmbodiedPromptPath`（v3）与 `FirstVersionEmbodiedPromptPath`（v1）并列存在，6 层 v3 contract + 11 字段拟人化 JSON schema + 7 条硬规则 cross-field invariants。LLM 身份块改写为"You are a person. Not an AI, not a role, not a runtime."。新增 11 个单元测试位于 tests/test_aggressive_radical_prompt_path.py。配套 R79-D 可扩展 baseline 框架（tests/r79d/）也同步交付：4 个 v1 场景（持续表扬/持续忽视/双相/重复刺激 20 tick）、9 个内建断言、CLI 入口、52 tick 真实 LLM 基线运行、aggregate.md 16/28 断言 PASS。R79-A / R79-D 均不改 owner 边界；R79-B / R79-C / R80 / R81 / R82 计划在 docs/requirements/79-r79-aggressive-radical-prompt-and-runtime-self-talk/ 需求包内追踪。默认装配 + v1 路径字节级不变；v3 路径经 `AggressiveRadicalPromptProfile(prompt_path_mode="aggressive-radical-v3")` 选择。测试基线：842 passed（831 基线 + 11 R79-A），0 回归；R21 ad-hoc logging guard 绿；composition owner-boundary guard 绿。版本：R79-A。）
