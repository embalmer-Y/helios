...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R79-C（Hormone-Predict Corroborator and 5-HT/Oxy/Opioid Drivers — `04` owner `helios_v2.neuromodulation` 现在为 serotonin / acetylcholine / oxytocin / opioid_tone 四条剩余通道提供 appraisal-derived 驱动（serotonin 走 `social * (1-uncertainty)` 反相、acetylcholine 走 `novelty * (1-fatigue)`、oxytocin / opioid_tone 走 `social * (1-uncertainty)` 与 `social * (1-threat)*(1-uncertainty)`，新增显式 `social_uncertainty_to_oxytocin = 0.4` / `safety_uncertainty_to_opioid = 0.4` 灵敏度常量）；第 5 个 `LearnedParameterCategory` literal `hormone_predict_coupling` 引入并应用到 `NeuromodulatorConfig` 校验；新 `04`-owned `helios_v2.neuromodulation.corroborator` 模块（`HormonePredictCouplingChannel` enum 9 通道 + `HormonePredictCouplingVerdict` 三态 + `HormonePredictCouplingConfig` + `HormonePredictCouplingClassification` + `HormonePredictCorroborator.classify_predict` + `aggregate_coupling_bias` 返回 `dict[str, float]` 而非 `NeuromodulatorLevels`，因为 conflict penalty 是负值而 `NeuromodulatorLevels` 强制 [0,1]）；v3 LLM prompt schema 加第 12 字段 `hormone_response_i_predict`（9 键 dict）+ 第 6 条 hard rule "no other fields"；R79-D framework 的 `inject_v3_prompt` 重写，从 R79-A `AggressiveRadicalEmbodiedPromptPath` 实时渲染 v3 system prompt（替代硬编码 v2-draft 文件），per-tick 用最近 tick 的 `result.stage_results` 喂入 4 个 `{placeholder}`。34 个新测试：`test_r79c_learned_param_category.py` 4 + `test_hormone_predict_corroborator_config.py` 9 + `test_hormone_predict_corroborator_classify.py` 10 + `test_hormone_predict_corroborator_aggregate.py` 5 + `test_r79c_hormone_coverage.py` 6。R79-C 需求包三件套位于 `docs/requirements/79c-r79c-hormone-predict-corroborator-and-5ht-oxy-opioid-drivers/`。测试基线：900 passed（866 R79-B 基线 + 34 new），2 个 pre-existing perf-flake；R21 ad-hoc logging guard 绿；composition owner-boundary guard 绿。R79-D 端到端 probe（NOOP gateway）显示 5-HT 全部非恒定（0.028-0.045 range）✅、Oxy 非恒定（0.012 range）✅、Opioid 接近恒定（0.003 range，salience aggregator `uncertainty=1.0` 默认吃光 `(1-uncertainty)` 因子——R80 composition scope）。版本：R79-C。）
