...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R79-B（Channel-Catalog Runtime Injection and LLM Channel Arbitration — `AggressiveRadicalPromptProfile` capability bundle（frozen dataclass，fail-fast `__post_init__` 校验空 channels / 重复 channels / 非字符串 channels）；`RuntimeProfile.aggressive_radical_prompt_profile` 字段接入 `assemble_runtime`（bootstrap id 切到 `v3-aggressive-radical` + 路径选择 `AggressiveRadicalEmbodiedPromptPath`）；`FirstVersionEmbodiedPromptRequestBridge` 和 `SemanticEmbodiedPromptRequestBridge` 加 `ready_channels: tuple[str, ...] = ()` class field，配合 `_resolved_channels` 投影（v1 默认回退到 `("cli",)` shim）；`AggressiveRadicalChannelArbitrationPostProcessor` owner-neutral glue 解析 v3 LLM JSON envelope 的 `i_will_send_it` / `i_send_through` / `i_want_to_say` 三元组并通过 `ChannelSubsystem.dispatch_outbound` 派发单个 `OutboundPacket`（5 类 fail-soft 原因：`parse_error` / `not_sending` / `channel_not_ready` / `empty_text` / `no_subsystem`；`AggressiveRadicalChannelArbitrationOutcome` frozen dataclass）。19 个新测试：13 个在 `tests/test_aggressive_radical_channel_arbitration.py`、6 个在 `tests/test_r79b_runtime_integration.py`。R79-B 需求包三件套位于 `docs/requirements/79b-r79b-channel-catalog-runtime-injection-and-llm-arbitration/`。默认装配 + v1 路径字节级不变；v3 路径经 `AggressiveRadicalPromptProfile(ready_channels=(...))` 选择。测试基线：866 passed（847 基线 + 13 仲裁 + 6 集成），0 回归；R21 ad-hoc logging guard 绿；composition owner-boundary guard 绿。版本：R79-B。）
