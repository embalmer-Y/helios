...
> 必须在同一次变更里同步更新本文件。
> 最近同步：**R97 — 去英文中心 / 中文 Appraisal Grounding（B3 根因闭合，ROADMAP §9.1）**。R96 闭合 B2（hash embedding 无语义）；R97 闭合 B3（R40 threat/reward 原型是英文，中文情绪输入 cosine ≈ 0，`03` appraisal 的 threat / reward 维对中文恒为 0）。R97 引入多语 `appraisal.anchor_catalog.AnchorCatalog`（frozen dataclass；`sets: tuple[AnchorSet, ...]`）；first-version `DEFAULT_ANCHOR_CATALOG` 是双语（5 中文 threat + 5 中文 reward + R40 英文 anchor 复用保持 R40 字节级）。`GroundedDimensionEstimator` 增 `anchor_catalog: AnchorCatalog` 字段（默认 `DEFAULT_ANCHOR_CATALOG` via `field(default_factory=...)` 打破 `engine` <-> `anchor_catalog` import 环）；`estimate_dimensions` 取 `max(R40 原型, catalog 维度)`。catalog 是 P5 learned-catalog 注入接缝（`AnchorCatalog(sets=learned_sets)`）。**owner 边界保留**：appraisal owner 仍拥有"哪一组是 threat / reward"的语义权威；composition 只注入 candidate 集合。R97 自动级联到 R96 真实 LLM 探针：`analyze.py` 加 `b3_closed_real_llm: bool | None`（B3 阈值 0.10 比 B2 的 0.05 更严）。17 个新测试（6 catalog 单元 + 8 estimator 装配 + 3 B3 closure）+ 6 份 B3 闭合报告写入 `helios_v2/logs/r97_b3_closure/`。**B3 网络无关闭合判定**：threat + reward shift 在 ≥ 8/10 中文 fixture；recall-over-recency 在 R97 路径通过、在 R40-only 路径失败；英文 anchor 字节级保留（R40 未被回退）。1126 passed / 4 skipped / 5 预存在 wall-clock-profile + lt1 失败。R56/R57 owner 边界 + R95-followup no-adhoc-logging guard 通过。先前同步 **R96 — Real Semantic Embedding as Default（B2 根因闭合，ROADMAP §9.1）**。R69 默认装配的 16-dim 字符哈希（结构占位、非脑模拟）被 OpenAI 兼容云端真实语义 embedding 取代为默认值（owner-confirmed decision A：`text-embedding-3-small` 默认，1536-dim）；该变更**纯加性 + 凭据级 opt-in** — 当 `HELIOS_EMBEDDING_API_KEY` 未设置时，R69 哈希路径逐字节保留。**项目级 env 加载统一**：`composition.embedding_provider_resolution` 是 provider 选择的单一来源；R82 的 `_production_embedding_gateway` 与 R69 自动装配块都委托给 `resolve_embedding_provider` + `build_embedding_gateway`（无重复常量、无漂移）。库代码通过可注入的 `env: Mapping[str, str]` 读 env；入口脚本调 `load_dotenv()`。`RuntimeProfile` 扩展 `embedding_provider_kind` / `embedding_provider_model` 两个可观察性字段（frozen + 校验）。19 个新测试（10 resolver 单元 + 6 R96 composition + 3 B2 shift）+ 6 份 B2 闭合报告写入 `helios_v2/logs/r96_b2_closure/`。**网络无关闭合判定**：novelty 在 10/10 fixture 上有 shift；prototype-cosine 在 10/10 fixture 上有 shift；recall-over-recency 在 fake-openai 通过、在 hash 失败。**真实 LLM opt-in 探针**位于 `helios_v2/scripts/r96_b2_real_llm_probes/`（`run.py` + `analyze.py` + README）；85 句离线烟测跑通（cortisol 正负情绪分离 = -0.0180，`b2_closed_real_llm: None` 因为哈希路径为当前激活 kind）。1112 passed / 4 skipped / 5 预存在 wall-clock-profile + lt1 失败。R56/R57 owner 边界 + R95-followup no-adhoc-logging guard 通过。先前同步 R95-followup（engine / planner 拆掉 legacy op-name 硬编码 — engine 离线默认 op 从 `request.prompt_contract_summary["available_channel_ops"]` 数据驱动读取，不再硬编 `reply_message`；planner 的 outbound-op 硬编集删除；first-version shim 是 `reply_message` 字面值的唯一允许携带位置；新增 `tests/test_no_hardcoded_op_names_in_engines.py` 回归测试）。1109 + R95 + R95-followup 新增 passed / 4 skipped。先前同步 R95（W2.6 行为中立 schema — schema 中所有行为暗示性关键词**彻底删除**：`reply_text` / `i_want_to_use_tool` / `wants_to_continue` / `intends_action` / `intends_revision` / `action_intent` / `target_user_id`；面向用户的面收窄到 5 个字段，新增 `thinking_complete`（新；语义反转）+ `channel_request`（新），`tool_op` **提升**为单一主动作类；`_emit_proposal` 折叠为单分支；self-revision 改为 OWNER-only；system prompt 新增 "Available channels" 区块，列出每个 driver × op；`reply_message` **不再特殊化**；composition 的 `_current_operator_id(frame)` 投影**彻底删除** — channel 不标记 `source_user_id`；身份 + 通道路由完全是 LLM 的内容决策）。**8/8 真实 LLM probe 全部 PASS**（deepseek/deepseek-v4-pro via shengsuanyun router，2026-06-15；probe 08 签名级跨通道路由验证：QQ 来消息 → 模型自主选 `qq.send_message` 而非 `cli.reply_message`）。先前同步 R94（W2.5：彻底移除 `i_want_to_say`；回复文本迁移到 `reply_text: str | None`；`_emit_proposal` 兼容路径删除；结构化测试强制断言无遗留字面值）+ R93 Phase 2（W2：action_intent / target_user_id / bound_user_ids / planner 优先级）+ R93（W2：i_want_to_say 回复环闭合）+ R92（W1 wall-clock）+ R91（W1 present-field）+ R90（P5 memory fidelity probe）+ R89（P5 图灵 harness）+ R88（P5 漂移评估器）+ R83（长跑稳定性 harness）。
...
- 认知主链（02 到 17）端到端贯通；996 测试全绿、离线，外加真实 LLM 冒烟。
...
- P4 自主工具闭环（R85）：FG-4 自主工具使用闭环收口。原则——**认知命名 op+params；每个 driver 自描述 op 属性；`13`/`14`/`17` 解释之**（`30` 设备自描述模型延伸到工具语义）。`30` descriptor 加 per-op `ChannelOpSpec`（`required_params`/`user_visible` 启用；`effect_class`/`risk_class` 声明,留 R86/R87）；CLI 声明 `reply_message`(user-visible,需 outbound_text+target),OS driver 声明各 `fs_*`(path/content,非user-visible,local_host)。`16` v3 schema 加工具意图字段;`11` 解析成 `external`-scope 工具提议(`op_params` 载荷,恶意/残缺降级为无提议、绝不编造);`12` 结构性归一化(D2a:外化的 op 感知校验上移 `13`,移除 `external⇒outbound_text` 规则);`13` 按 driver 声明的 `required_params` **通用校验**(reply 与 `fs_*` 统一,缺键→`missing_op_inputs`,无 spec 时回落旧 reply 检查),能力门由 channel-state 派生(连接 driver 声明该 op→registered;未声明→`behavior_not_registered`),op 的 effect/risk class 走 policy trace(read-through)。另收口 planner-rejection tick:`policy_rejected`/`execution_consistency_failed` 写 `world_blocked` 连续性记录(FG-4.4,失败正式写回),autonomy/evaluation 仍跑。端到端(channel-bound,CLI 触发+OS driver):operator 一行触发 tick→模型断言 `fs_write`→planner 绑定 `os_fs`→dispatch 沙箱内执行→结果下一 tick 作为 `tool_result` 回流 `02`,确定性 provider 网络无关验证。无 function-calling(`25` 不变);认知绝不选/绑/校验 channel;per-op 知识归 driver。默认/reply/no-action 路径不变。912→921 测试绿。
