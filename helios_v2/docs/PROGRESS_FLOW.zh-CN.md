...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R86（受治理 OS 命令执行 effector + `13` 强制 risk-class 门 + `14` 两-tick fail-closed governed-action 授权握手：新 `helios_v2.channel.drivers.os_command`（`run_command`，op 级 `risk_class="governed"`，argv-前缀 default-deny allowlist——`unrestricted` 只读/诊断 + `governed` `mkdir`/`cp`/`mv`），no-shell/sandbox cwd/超时，注入式 `CommandExecutor`+`CommandRunner`（CI 用 `FakeCommandRunner` 无子进程），`tool_result` 回流。`13` 把 R85 的 `risk_class` read-through 升级为**强制 fail-closed 门**：op 级 `unrestricted`（reply/`fs_*`）字节级不变、`restricted`/未知命令 → `risk_class_restricted` 硬拒、`governed` → 查 carried `14` 授权 → 放行 / `governance_denied` / `governance_required`。`14` additive 扩展为 governed 动作授权权威（`GovernedActionAuthorization` + `GovernedActionGovernancePath` + `PriorGovernedAuthorizationHolder` 两-tick carry；自我修订路径不变）。解释器/写自身代码永久 restricted。无 owner 新增、阶段链不变（仍 21 阶段 channel-bound）。测试基线：957 passed / 4 skipped（离线）。先前同步 R85（FG-4 自主工具闭环：`11` 真实认知选工具 op+params → `12` 结构性归一化 → `13` 按 op 绑定 driver + 通用 `required_params` 校验 + 能力门由 channel-state 派生 → 执行 → 结果回流 `02` 再思考。每个 driver 自描述 per-op `ChannelOpSpec`（required_params/user_visible 启用，effect_class/risk_class 声明）；取消 `tool` scope，op 感知校验从 `12` 上移 `13`；planner-rejection tick 新增 `world_blocked` 写回闭环。无 owner 新增、阶段链不变。测试基线：921 passed / 4 skipped（离线）。先前同步 R84（P4 开篇：首个 effector driver `helios_v2.channel.drivers.os_fs`（沙箱化 OS 文件 driver，`fs_read/write/list/modify`），工具结果作为 `tool_result` 带 correlation provenance 回流 `02`（efference→reafference 闭环，FG-4）；注入式 executor（测试 inline / 生产 ThreadPool 真异步）；`send_outbound` 同步受理异步执行；channel-bound 装配泛化为 `RuntimeProfile.channel_drivers` 可注册一组 driver（CLI+effector 共存）。无新 owner、阶段链不变（仍 21 阶段 channel-bound）。测试基线：912 passed / 4 skipped（离线）。版本：R86。
