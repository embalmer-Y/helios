...
> 必须在同一次变更里同步更新本文件。
> 最近同步：R92（Wall-Clock 真实时间戳——新增基础设施 owner `helios_v2.wall_clock`：单方法 `WallClock` 协议 + `SystemWallClock`（`time.time()` 懒导入）+ 测试用 `FixedWallClock` + 保留元数据键 `RECEIVED_AT_WALL_METADATA_KEY`；纯事实源、不持任何认知策略、不 import 任何认知 owner。三处 additive 消费：`RuntimeFrame.tick_wall_seconds` 由内核每 tick 起种入；`CliChannelDriver.submit_line` 戳 `received_at_wall` 入 `InboundPacket.metadata`（到达时而非 drain 时，经 `02` 透传到 `Stimulus.metadata`）；`PersistedExperienceRecord.created_at_wall` 经两条 record bridge 写入（SQLite 经 PRAGMA-guarded `ALTER TABLE` 就地迁移；旧文件读回 `None`）。R91 `_present_field_summary_text` 多出一个 `last input: <X.X>s ago` clause（与 `pacing: <signal>` 并列；NTP-rewind 钳到 `0.0s`；缺任一端则诚实省略）。`RuntimeProfile.wall_clock` 是 opt-in capability 接缝；`assemble_production_runtime` 默认开启 `SystemWallClock`，`assemble_runtime` 默认 None 字节级不变。**owner 数 +1（infra `helios_v2.wall_clock`，蓝色），认知阶段链/owner 边界/既有 owner 成熟度均无变化**。测试基线：≥ 1059 passed / 4 skipped（离线）。先前同步 R90（P5 评估框架完成——R88/R89/R90 均为 `tests/` 下只读 tests-only 诊断，无 owner/阶段链/边界变更）。
