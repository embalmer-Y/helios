...
> 状态：活文档（owner 参考）。最近同步：**R101 — 6 维客观重要性 + 双重确认 + P5 基础（2026-06-17）**。测试基线：R100 1329 passed / 4 skipped + R101 165 passed = ~1500+ passed / 0 failed。  **R96–R101（W3 真实语义 + W4 情感验收 + R100/R101 双轨记忆前两片，2026-06）已交付**：R96 闭合 B2（OpenAI-compatible 云端真实 embedding 替换 hash，1536-dim `text-embedding-3-small`，凭据级 opt-in）；R97 闭合 B3（多语 `AnchorCatalog` 双语锚点：5→11 中文 threat + 5 中文 reward + R40 英文复用）；R98 scope extension（hormone prediction prompt coaching）；R99 W4 情感验收探针正式化；R100 双轨记忆第一片（`MemoryRecord` + 4 层时间分层 + `AffectOutcomeMemoryLayerClassifier` 写入时刻决定初始层 + `PersistedExperienceRecord.layer`/`memory_metadata` 加性 + SQLite PRAGMA-guarded `ALTER TABLE` + `RuntimeProfile.memory_layer_classifier`/`memory_layer_preference` 纵缝 + semantic 装配接 `("L4_long", "L5_autobiographical")`）；R101 双轨记忆第二片（6 维 `ObjectiveImportanceVector` 刺激强度/皮质醇/唤醒/结局权重/新颖度/关系风险 + `OUTCOME_CLASS_WEIGHTS` 8 类脑模型平行 HPA 轴/皮层唤醒/结局归类 + `DoubleConfirmationGate` 4 类（both_pass/objective_only/subjective_only/skip 阈值 0.50/0.60）+ `ObjectiveImportanceLayerResolver` skip→L2 + self_changed L2→L5 PROMOTE=0.70/DEMOTE=0.85 + `RecallUtilityTracker` RECALL_EMA_ALPHA=0.3 + `MemoryImportanceLoss` Protocol + `SqlBackedTrainingDatasetExtractor` 6 SQL 过滤 + `MemoryRecord` 增 9 字段 + `PersistedExperienceRecord` 增 8 字段（1 JSON + 4 indexed + 3 utility）+ SQLite `ALTER TABLE` 增 8 列 + `MemoryAffectReplayEngine` 增 6 注入点 + `r101_active` 分支（任一 seam None → legacy R100 路径字节级）+ `RuntimeProfile` 增 6 R101 纵缝 + all-or-nothing 验证）。先前 R95 + R95 followup C1-C6 全部已交付。
