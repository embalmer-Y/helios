...
> 状态：活文档（owner 参考）。最近同步：R39。测试基线：523 passed（离线）。
...
- 完成度：`baseline_real`（语义记忆装配下五维中三维已真实：novelty 自 R35、uncertainty + social 自 R39；threat/reward 仍 shim,待 R40）。
...
- 下一步（本 owner 的分阶段 P3 去 shim）：
  ...
  4. **R40 — threat/reward（下一刀）。** 用网络无关的原型 embedding 法去 shim：装配期 embed 一小组威胁/奖励原型短语,各维按刺激对原型集的最大余弦打分。仍在快路,但 grounding 为 `C_engineering_hypothesis`（原型集是人工、英语中心的设计选择），须详尽标注、绝不过度宣称；原型集是后续学习切片或 `06` 记忆-情感 grounding 替换的接口。基于 LLM 的慢速 threat/reward 再评估是独立的二级 owner 关切,不在快 `03` 路。
  5. **聚合显著性。** 维度变真后，把常量 aggregate estimator 换成可学习或模型辅助的整体判断。
  6. **下游耦合（novelty→`04`→`05`/`09` 已交付）。** 真实 `03` 显著性已塑造 `04` 神经调质状态（R36），并经其塑造 `05` 体感（R38）与 `09` 门控（R37）。仍待：去 shim 的 threat/reward 喂给 cortisol/inhibition 硬门控,以及它们落地后更丰富的 `05`/`06` 耦合；之后做情感加权或时近加权。
