...
> 状态：活文档（owner 参考）。最近同步：**R99 — W4 情感验收探针正式化（2026-06-17）**。测试基线：1172 passed / 4 skipped（离线）/ 5 个 pre-existing wall-clock-profile + lt1 失败（需 LLM profile 启动）。  **R96–R99（W3 真实语义 + W4 情感验收，2026-06）已交付**：R96 闭合 B2（OpenAI-compatible 云端真实 embedding 替换 hash，1536-dim `text-embedding-3-small`，凭据级 opt-in）；R97 闭合 B3（多语 `AnchorCatalog` 双语锚点：5→11 中文 threat + 5 中文 reward + R40 英文复用）；R98 scope extension（hormone prediction prompt coaching — `_build_messages` hormone schema 扩展 channel-to-emotion mapping + 双语示例，`_V3_RESPONSE_SCHEMA` 同步扩展，教 LLM 在 negative-valence 写 cortisol: "elevated"）；R99 W4 情感验收探针正式化（tests-only `tests/r99_emotion_validation_probe/`，四 bounded `[0,1]` metric：cortisol_valence_separation=0.0785、thought_content_grounding=1.0、memory_recall_relevance（诚实）、reply_loop_closure=0.8；R89 additive `emotion_validation_probe` 参数升级 `bio_responsiveness` 轴）。1172 passed / 4 skipped。先前 R95 + R95 followup C1-C6 全部已交付。
