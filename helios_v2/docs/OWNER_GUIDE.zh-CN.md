...
> 状态：活文档（owner 参考）。最近同步：R95-followup（engine / planner 拆掉 legacy op-name 硬编码）。测试基线：1109 + R95 + R95-followup 新增 passed / 4 skipped（离线）/ 5 个 pre-existing wall-clock-profile + lt1 失败（需 LLM profile 启动）。  **R95（W2.6 行为中立 schema，2026-06）— 模型对 action + channel 拥有完整自主权**：彻底删除 schema 中所有行为暗示性关键词（`reply_text` / `i_want_to_use_tool` / `wants_to_continue` / `intends_action` / `intends_revision` / `action_intent` / `target_user_id`）。动作是单一主字段（`tool_op`）；续推是 `thinking_complete`；模型可通过 `channel_request` 申请新通道。composition 的 `_current_operator_id` 投影已删除 — channel 不负责标记 `source_user_id`；身份完全是 LLM 的内容决策。system prompt 暴露完整的 "Available channels" 区块（每个 driver × 每个 op，列出 `op_name` / `required_params` / `effect_class` / `risk_class` / `bound_user_ids`）；`reply_message` 不再特殊化。self-revision 改为 OWNER-only（autobiographical 命中 + `sufficiency >= 0.75`），不再有 `intends_revision` 字段。6 个新 R95 测试文件（30 tests）+ 8 个真实 LLM probe 全部 PASS（deepseek/deepseek-v4-pro via shengsuanyun router，2026-06-15）。  **R95 followup C1-C5（2026-06）— engine / planner 拆掉 legacy op-name 硬编码**：engine 离线默认 op 从 `request.prompt_contract_summary["available_channel_ops"]` 数据驱动读取，不再硬编 `reply_message`；planner 的 outbound-op 硬编集 `{"reply_message", "send_message", "speak_text"}` 已删除；first-version shim（`composition/bridges.py` 的 `_FIRST_VERSION_SYNTHETIC_CLI_OPS`）是 `reply_message` 字面值的**唯一**允许携带位置（shim 的显式单 op 政策）；生产 semantic assembly 投影真 driver state 不带字面。回归测试 `tests/test_no_hardcoded_op_names_in_engines.py`（3 tests）守住——`internal_thought/engine.py` / `planner_bridge/engine.py` / `action_externalization/engine.py` 任一文件代码部分（剥离注释/docstring）出现 op-name 字面即失败。
...
- **R93（W2 对话回复闭环可靠化）**：`_parse_structured_thought` 读可选顶层 `i_want_to_say` 字段（缺省/null/空/纯空白→`""`；非空 trim 保留；超 2000 字符 deterministic 截断带 `…(truncated)` 后缀；非字符串→`StructuredThoughtParseError`），存到新增 `StructuredThoughtEvidence.intended_reply_text: str = ""`（默认空保留 pre-R93 字节级）。`_emit_proposal` 在 `evidence is not None` 且无显式 `tool_op` 且 `intended_reply_text` 非空且 prompt-contract 摘要的 `current_operator_id` 非空时，构造一个**隐式 `reply_message` tool intent**（`op_params={"outbound_text": <reply>, "target_user_id": <operator>}`，`preferred_channels` 取自 `ready_channels`，`outbound_text=None`）走 R85 既有 planner-spine——`13` 校验 + 绑定 + `channel_outbound_dispatch` 真正送达 CLI sink。显式 `tool_op`（R85）始终优先；无 `current_operator_id`（interooceptive-only tick）静默不构造（绝不虚构目标）；`evidence is None`（决定性 offline 路径）字节级不变。`_build_messages` 把 `i_want_to_say` 加入 schema 行并附一句"模型：填此字段时 runtime 会通过 cli 把它作为 `reply_message` 发给当前操作者"的 transport clause。owner 边界：`11` 仍保留全部判断；composition 只读已发布 `02` stage 结果并正向投影 `current_operator_id`（不动 `06`/`10`/`13`）；不新增 logging；不动 owner 色/链/边界。
- **R93 Phase 2（W2 行动自主 + 跨通道路由，2026-06）**：envelope 新增 additive `action_intent: 'reply' | 'tool' | 'no_action'` 与 `target_user_id: str | None` 字段。`_emit_proposal` 优先级改写：显式 `tool_op` 胜出；`action_intent="tool"` 胜出；`action_intent="reply"` + 解析得到 target 构造 `reply_message` 提议（R93 P2 显式路径；R93 P1 的 `i_want_to_say + operator` 兼容路径保留作为 Phase-1 迁移援助）；`action_intent="no_action"` 内部-only 收口，覆盖所有其他信号。`target_user_id` 解析：模型声明 > composition 投影的 `current_operator_id`。`ChannelOpSpec.bound_user_ids` 让 driver 自报其服务的 user IDs（CLI 声明 `frozenset()` 通配）。planner `_select_channel` 优先级改写为 `target_user` -> `preferred` -> `iteration-order`。旧 `emit_action` fallback 删除；reply 不再是隐式默认。
- **R94（W2.5 移除 i_want_to_say，2026-06）**：R93 P1 的 `i_want_to_say` envelope 字段**彻底移除**。回复文本迁移到 `reply_text: str | None`（`action_intent="reply"` 的子细节）。`_emit_proposal` 兼容路径（`reply_text` 设置但无 `action_intent`）删除。结构化测试 `test_internal_thought_no_i_want_to_say_in_prompt.py` 在 system prompt / parser / fixture 三个层面强制断言无 `i_want_to_say` 字面值。通道路由（R93 P2 `bound_user_ids` + planner 优先级）保持不变。R94 是对 R93 P1 字段名偏见的严格清理；决定性 offline 路径字节级不变。
- **R95（W2.6 行为中立 schema，2026-06）**：schema 中的所有行为暗示性关键词**彻底删除**。具体而言，envelope 失去 `reply_text` / `i_want_to_use_tool` / `wants_to_continue` / `intends_action` / `intends_revision` / `action_intent` 以及隐式 `target_user_id` 字段。面向用户的面收窄到 5 个字段：`thought` / `sufficiency` / `thinking_complete`（新；取代 `wants_to_continue` 但语义反转——`True` = 想完了）/ `tool_op`（**提升**为单一主动作类字段）/ `tool_params` / `channel_request`（新；模型可描述想要但当前不存在的 channel/op）/ 沿用的 `hormone_response_i_predict`。`_emit_proposal` 折叠为单一分支：`if evidence.tool_op:` 构造提议；否则 cycle 内部收口。self-revision 改为 OWNER-only（autobiographical 命中 + `sufficiency >= 0.75`），模型的 `intends_revision` 字段消失。system prompt 新增 "Available channels" 区块，列出每个 driver × 每个 op 的 `op_name` / `required_params` / `effect_class` / `risk_class` / `bound_user_ids`；`reply_message` **不再特殊化**——它只是 `cli` driver 众多 op 中的一个，与其他 op 并列。composition 的 `_current_operator_id(frame)` 投影**彻底删除**；桥改为投影 `available_channel_ops`（从 `frame.channel_state` 读）。channel **不负责**标记 `source_user_id`（CLI 的 `send_outbound` 不用 `target_user_id`）；身份 + 通道路由是 LLM 的内容决策，不是 runtime 启发式。验证：6 个新 R95 测试文件（共 30 tests）：`test_internal_thought_no_behavior_suggestive_in_prompt.py`（4）/ `test_internal_thought_thinking_complete_field.py`（4）/ `test_internal_thought_channel_request_field.py`（3）/ `test_internal_thought_available_channels_in_prompt.py`（4）/ `test_internal_thought_no_target_user_id_projection.py`（3）/ `test_internal_thought_emit_proposal_r95.py`（8）。加 8 个真实 LLM probe 于 `helios_v2/scripts/r95_probes/01..08_*.json` —— **8/8 全部 PASS**（deepseek/deepseek-v4-pro via shengsuanyun router，2026-06-15；详见 `docs/requirements/95-behavior-neutral-schema/probe_results.md`）。
- **R95 followup C1-C5（engine / planner hardcode 清理，2026-06）**：engine 离线默认 op 不再硬编。新增 `_default_op_from_request(request)` helper，从 `request.prompt_contract_summary["available_channel_ops"]` 读第一个 op 的 `op_name`（数据驱动）；当无可用 op 时 cycle 内部收口（`action_proposal = None`）。R93 P1 的 `behavior_name="reply_message"` byte-for-byte acceptance **改为数据驱动等价**——shim 的 first-version 装配 + 测试夹具现在在 `available_channel_ops` 里填入 synthetic cli 默认，所以离线路径仍可测但 engine 不知道 op 名。planner 的 `_missing_required_input` 不再带 hardcoded `{"reply_message", "send_message", "speak_text"}` 集；op spec 缺失即"无 required-param 契约"（channel driver 是唯一 source of truth）。`composition/bridges.py` 的 first-version shim 是**唯一**允许携带 `reply_message` 字面值的位置（`_FIRST_VERSION_SYNTHETIC_CLI_OPS`，shim 显式单 op 政策）；生产 semantic 装配投影真 `frame.channel_state` 不带字面。验证：`tests/test_no_hardcoded_op_names_in_engines.py`（3 tests）——`internal_thought/engine.py` / `planner_bridge/engine.py` / `action_externalization/engine.py` 任一文件代码部分（剥离注释/docstring）出现 op-name 字面值即失败。owner 边界：`11` 和 `13` 现在都将 op 名视为 driver 自描述返回的不透明字符串；不知道哪个 op 是"outbound"或"reply"——那是 driver 的契约。
...
- **R93（W2 对话回复闭环可靠化，`13` 校验链不变）**：`13` 继续保持 `required_params` 校验（reply_message 需要 `outbound_text` + `target_user_id`）；R93 不在 `13` 内构造内容，只让 R85 既有校验链接受 `11` 新增的隐式 reply tool intent（`op_params` 携带真实回复文本 + 当前操作者）。防御纵深：`13` 拒绝任何缺字段的隐式 reply（与 R85 既有规则一致）；命令 op 的 `risk_class` fail-closed 门不变。
