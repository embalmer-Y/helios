...
> 状态：活文档（owner 参考）。最近同步：R95。测试基线：1106 + R95 新增 passed / 4 skipped（离线）/ 5 个 pre-existing wall-clock-profile + lt1 失败（需 LLM profile 启动）。  **R95（W2.6 行为中立 schema，2026-06）— 模型对 action + channel 拥有完整自主权**：彻底删除 schema 中所有行为暗示性关键词（`reply_text` / `i_want_to_use_tool` / `wants_to_continue` / `intends_action` / `intends_revision` / `action_intent` / `target_user_id`）。动作是单一主字段（`tool_op`）；续推是 `thinking_complete`；模型可通过 `channel_request` 申请新通道。composition 的 `_current_operator_id` 投影已删除 — channel 不负责标记 `source_user_id`；身份完全是 LLM 的内容决策。system prompt 暴露完整的 "Available channels" 区块（每个 driver × 每个 op，列出 `op_name` / `required_params` / `effect_class` / `risk_class` / `bound_user_ids`）；`reply_message` 不再特殊化。self-revision 改为 OWNER-only（autobiographical 命中 + `sufficiency >= 0.75`），不再有 `intends_revision` 字段。6 个新 R95 测试文件（30 tests）+ 8 个真实 LLM probe（延期：API key 轮换）。
