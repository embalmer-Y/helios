...
> Status: living owner reference. Last synced: R55. Test baseline: 702 passed (network-free).
...
- Next step (P2/P3): (1) ✅ formation de-shim + durable store — **delivered in R45**; (2) ✅ recall prior affect-memories as additional replay candidates feeding `07` — **delivered in R52** (recalled-replay surfacing: `06` re-forms semantically-recalled past affect-memories into non-forced replay candidates through an injected `RecalledMemoryProvider`, with an owner-owned recall-relevance + affect-intensity priority mapping, giving `07` its first genuine multiplicity); (3) deduplication and same-memory merge/consolidation (under `consolidation_policy`, bounded, owner-owned), retiring this slice's no-dedup constraint; (4) drive formation from the real `05` feeling beyond the affect tag; (5) same-register novelty once R35 method B (persist the raw stimulus) lands.
...
- Completeness: `baseline_real` (two inputs now real under the semantic-memory assembly: arousal (R37) + global activation (R48); the rest remain shim).
...
5. The owners genuinely driven by real signals today are `02`, `08`, `11`, `18`, plus all infrastructure owners; everything else is honest baseline awaiting its de-shim wave. Under the semantic + interoceptive assembly `03-08` are de-shimmed, and R51 closed the first end-to-end FG-2 causal chain (real pressure → `05` feeling → `07` competition).
