...
- Next step: (1) dual-timescale feeling persistence carrying prior-tick feeling (the declared `feeling_persistence` category; depends on a feeling-state carry/checkpoint, the `18`/`09`/`14`/`04` family); (2) integrate real internal body/interoceptive signals once a real interoceptive source exists — note this is currently an unowned gap (`gap_interoceptive_signal_source`): `05` already filters the `02` batch for body/interoceptive modality, but no owner produces such signals (the `BODY` node in the progress maps is an interface-only placeholder), so a future producer is needed (a simulated body-state model, or a compute/runtime-pressure proxy); (3) `P5` learning of the bounded coupling coefficients; (4) feed the real feeling into `06` memory-affect tagging, conscious content, and behavior so body-state causally matters downstream (FG-2); (5) de-shim the remaining `03` dimensions and `04`'s dual-timescale dynamics so all upstream drivers are real.
