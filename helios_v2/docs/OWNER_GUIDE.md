...
> Status: living owner reference. Last synced: R39. Test baseline: 523 passed (network-free).
...
- Completeness: `baseline_real` (three of five dimensions real under the semantic-memory assembly: novelty since R35, uncertainty + social since R39; threat/reward still shim pending R40).
...
- Next step (staged P3 de-shim of this owner):
  ...
  4. **R40 — threat/reward (next slice).** De-shim `threat`/`reward` via a network-free prototype-embedding method: embed a small assembled set of threat/reward prototype phrases and score each dimension by the stimulus's maximum cosine to the prototype set. This stays in the fast path, but its grounding is `C_engineering_hypothesis` (the prototype set is a hand-authored, language-centric design choice) and must be annotated thoroughly and never over-claimed; the prototype set is the surface a later learning slice or a `06` memory-affect grounding replaces. A slow LLM-based re-appraisal of threat/reward is a distinct second-stage owner concern, not the fast `03` path.
  5. **Aggregate salience.** Replace the constant aggregate estimator with a learned or model-assisted overall judgment once the dimensions are real.
  6. **Downstream coupling (shipped for novelty→`04`→`05`/`09`).** Real `03` salience already shapes the `04` neuromodulator state (R36) and through it `05` feeling (R38) and `09` gating (R37). Remaining: feed the de-shimmed threat/reward into the cortisol/inhibition hard gate and richer `05`/`06` coupling once they land; later affect-weight or recency-weight the dimensions.
...
- Completeness: `baseline_real` (feeling now neuromodulator-derived under the semantic-memory assembly; stateless).
...
- Completeness detail: with `38` (P3 fourth de-shim) the constant construction shim is replaced under the semantic-memory assembly by the owner-private `NeuromodulatorDerivedFeelingConstructionPath` (the channel→dimension mapping lives in `05` itself, since subjectivizing neuromodulator state into feeling is this owner's whole reason to exist; the engine and contracts are unchanged, no new bridge, no stage reorder). Each dimension is `clamp(baseline + sum(coupling_k * level_k), legal_min, legal_max)`: valence +DA/opioid/5-HT −cortisol, arousal +NE/excitation, tension +cortisol/NE, comfort +opioid/oxytocin/5-HT −cortisol, pain_like +cortisol −opioid, social_safety +oxytocin/5-HT −cortisol, fatigue +inhibition −excitation (weak). Deterministic, bounded (clamped to legal range), and **stateless** (no prior-tick feeling carried). With `37`, both of `04`'s downstream consumers (`09` gating and `05` feeling) now consume the real `04` state. Default/recency/offline assemblies keep the constant feeling. Caveat: only novelty is a real `03` driver today, the real `05` feeling does not yet measurably shape `06`/behavior, and real interoceptive `internal_signals` are not yet integrated.
- Next step: (1) dual-timescale feeling persistence carrying prior-tick feeling (the declared `feeling_persistence` category; depends on a feeling-state carry/checkpoint, the `18`/`09`/`14`/`04` family); (2) integrate real internal body/interoceptive signals once a real interoceptive source exists; (3) `P5` learning of the bounded coupling coefficients; (4) feed the real feeling into `06` memory-affect tagging, conscious content, and behavior so body-state causally matters downstream (FG-2); (5) de-shim the remaining `03` dimensions and `04`'s dual-timescale dynamics so all upstream drivers are real.
