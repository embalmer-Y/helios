...
> Last synced: **R97 — 去英文中心 / 中文 Appraisal Grounding (B3 root-cause closure, ROADMAP §9.1)**. R96 closed B2 (hash embedding noise); R97 closes B3 (R40 threat/reward prototype phrases are English-only, so Chinese emotion inputs score near-zero cosine and the `03` appraisal owner's `threat` / `reward` dimensions collapse to noise on Chinese input). R97 introduces a multilingual `appraisal.anchor_catalog.AnchorCatalog` (frozen dataclass; `sets: tuple[AnchorSet, ...]`); the first-version `DEFAULT_ANCHOR_CATALOG` is bilingual (5 中文 threat + 5 中文 reward + the R40 English anchors aliased to keep the R40 path byte-level equivalent). The `03` appraisal owner's `GroundedDimensionEstimator` gains an `anchor_catalog: AnchorCatalog` field (default via `field(default_factory=...)` to break the `engine` <-> `anchor_catalog` import cycle); `estimate_dimensions` takes `max(R40 prototypes_max, catalog_dimension_max)` for both threat and reward. The catalog is the clean P5 learned-catalog replacement seam (`AnchorCatalog(sets=learned_sets)` injected at the same composition seam). Owner boundary preserved: the appraisal owner keeps "which dimension is threat / reward" semantic authority; composition only injects the catalog as a candidate set. R97 cascades to the R96 real-LLM probe automatically: `analyze.py` adds `b3_closed_real_llm: bool | None` (stricter 0.10 threshold than B2's 0.05). 17 new tests (6 catalog unit + 8 estimator wiring + 3 B3 closure) + 6 B3 closure reports written to `helios_v2/logs/r97_b3_closure/`. **B3 network-free closure verdict**: threat + reward shift on ≥ 8 of 10 中文 fixtures; recall-over-recency holds for the R97 catalog-augmented path and fails for the R40-only path; English-anchor byte-level preserved (R40 not regressed). 1126 passed / 4 skipped / 5 pre-existing wall-clock-profile + lt1 failures. R56/R57 owner-boundary + R95-followup no-adhoc-logging guards pass. Previously: **R96 — Real Semantic Embedding as Default (B2 root-cause closure, ROADMAP §9.1)**. The R69 default assembly's 16-dim character hash (a structural placeholder, not a brain analog) is replaced by a real-semantic OpenAI-compatible cloud embedding as the default (owner-confirmed decision A: `text-embedding-3-small` default, 1536-dim); the change is **purely additive and opt-in at the credential level** — when `HELIOS_EMBEDDING_API_KEY` is absent, the R69 hash path runs unchanged. **Project-wide env-loading unification**: `composition.embedding_provider_resolution` is the single source of truth for provider selection; R82's `_production_embedding_gateway` and the R69 auto-provisioning block in `assemble_runtime` both delegate to `resolve_embedding_provider` + `build_embedding_gateway` (no duplicate constants, no drift). Library code reads env via injectable `env: Mapping[str, str]`; entry-point scripts use `load_dotenv()`. `RuntimeProfile` extended with `embedding_provider_kind` / `embedding_provider_model` observability fields (frozen, validated). 19 new tests (10 resolver unit + 6 R96 composition + 3 B2 shift) + 6 B2 closure reports written to `helios_v2/logs/r96_b2_closure/`. **Network-free closure verdict**: novelty shift on 10/10 fixtures, prototype-cosine shift on 10/10 fixtures, recall-over-recency passes for fake-openai and fails for hash. **Real-LLM opt-in probe** at `helios_v2/scripts/r96_b2_real_llm_probes/` (`run.py` + `analyze.py` + README); full 85-utterance offline smoke pass (cortisol positive-vs-negative separation = -0.0180, `b2_closed_real_llm: None` because the hash path is the active kind). 1112 passed / 4 skipped / 5 pre-existing wall-clock-profile + lt1 failures. R56/R57 owner-boundary + R95-followup no-adhoc-logging guards pass. Previously: R95-followup (engine/planner hardcode cleanup — the engine's offline default derives from `request.prompt_contract_summary["available_channel_ops"]` instead of a hardcoded `reply_message`; the planner's hardcoded outbound-op set is REMOVED; the first-version shim is the only allowed carrier of the literal `reply_message`; `tests/test_no_hardcoded_op_names_in_engines.py` regression guard added). 1109 + R95 + R95-followup new tests passed / 4 skipped. R95 (W2.6 behavior-neutral schema — the schema's behavior-suggestive keywords are entirely REMOVED: `reply_text` / `i_want_to_use_tool` / `wants_to_continue` / `intends_action` / `intends_revision` / `action_intent` / `target_user_id`; the user-facing surface is reduced to 5 fields with `thinking_complete` (NEW; inverted semantics) + `channel_request` (NEW) added and `tool_op` PROMOTED to single primary action-class; `_emit_proposal` is a single branch; self-revision becomes OWNER-only; the system prompt exposes an "Available channels" section listing every driver × op; `reply_message` is NOT special-cased; the composition's `_current_operator_id(frame)` projection is REMOVED — channels do not mark `source_user_id`; identity + channel routing is the LLM's content decision. **8/8 real-LLM probes PASS** (deepseek/deepseek-v4-pro via shengsuanyun router, 2026-06-15; probe 08 signature cross-channel routing test verified: inbound on QQ → model autonomously picked `qq.send_message`, not `cli.reply_message`). R94 (W2.5: drop i_want_to_say; reply text moves to `reply_text: str | None`; the `_emit_proposal` compat path is REMOVED; structural test enforces absence of the legacy literal in the system prompt / parser / fixture). R93 P2 (W2: action_intent / target_user_id / bound_user_ids / planner priority). R93 (W2: i_want_to_say reply loop closure). R92 (W1 wall-clock). R91 (W1 present-field). R90 (P5 memory fidelity probe). R89 (P5 Turing harness). R88 (P5 drift evaluator). R83 (long-run stability harness).
...
- Cognition main chain (02 to 17) runs end to end; 996 tests pass, network-free, plus real
  LLM smoke.
