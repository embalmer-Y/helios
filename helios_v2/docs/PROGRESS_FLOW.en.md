...
> Last synced: R85-A (Track A infrastructure — MemoryRecord 4-layer schema + objective_importance 6-dim + double confirmation + L3/L4 layer classifier + opt-in R85MemoryClassifierBridge; the `06` memory owner now exposes `MemoryRecord` / `MemoryLayer` / `DoubleConfirmationClass` / `OUTCOME_CLASS_WEIGHTS` plus the pure functions `should_persist` (7-branch decision tree with AND fallback) / `effective_priority` (Ebbinghaus decay + recency rebound) / `soft_delete_memory_record` (soft-delete with `soft_deleted_at` + `memory_gc_after` + audit trail) / `migrate_persisted_to_memory_v2` (backward-compatible from R15 PersistedExperienceRecord) / `objective_importance` (6-dim: 0.25*intensity + 0.20*cortisol + 0.15*arousal + 0.15*outcome_weight + 0.15*novelty + 0.10*(1-social_safety), clamped [0,1], optional `embed_callable` for cosine novelty with safe fallback) / `promote_layer` (L3→L4 at recall_count ≥ 2; L4→L5 at recall_count ≥ 5 + obj ≥ 0.7); a new `06`-owned `helios_v2.memory.classifier` module wraps both into `classify_for_persistence` (returns `MemoryClassification` with classification / objective_importance / target_layer / reason) + `make_memory_record` (pure factory, no clock reads / no I/O, R85 fail-fast); composition glue `R85MemoryClassifierBridge` in `composition/bridges.py` is an opt-in bridge that reads `PersistedExperienceRecord` items from `ExperienceRecordBridge.build_records` and emits `MemoryRecord` items, with explicit `hormone_snapshot` / `feeling_snapshot` / `llm_remember_per_record` / `created_at_wall` parameters (no automatic `assemble_runtime` integration — that wiring is deferred to Track B owner 31 mandatory driver); 61 new tests: `test_r85_memory_record.py` 27 + `test_r85_objective_importance.py` 18 + `test_r85_classifier.py` 11 + `test_r85_bridge_integration.py` 5. Test baseline: 1043 passed (987 R84 baseline + 56 net new; 2 pre-existing perf-flakes), R21 ad-hoc logging guard green, composition owner-boundary guard green. R85 progress: 0% → 35% (Track A infrastructure complete; Track B channel driver + Track C runtime wiring remaining). HEAD-era: R85-A.) — the `04` owner `helios_v2.neuromodulation` now provides appraisal-derived drives for the four remaining shimmed channels (serotonin via `social * (1-uncertainty)` inverse, acetylcholine via `novelty * (1-fatigue)`, oxytocin / opioid_tone via `social * (1-uncertainty)` and `social * (1-threat)*(1-uncertainty)` respectively; new explicit `social_uncertainty_to_oxytocin = 0.4` / `safety_uncertainty_to_opioid = 0.4` sensitivity constants under the existing `sensitivity_per_salience_dimension` P5-learnable category); the 5th `LearnedParameterCategory` literal `hormone_predict_coupling` is introduced and applied to `NeuromodulatorConfig` validation; a new `04`-owned `helios_v2.neuromodulation.corroborator` module (`HormonePredictCouplingChannel` enum with 9 channels + `HormonePredictCouplingVerdict` 3-state + `HormonePredictCouplingConfig` + `HormonePredictCouplingClassification` + `HormonePredictCorroborator.classify_predict` + `aggregate_coupling_bias` returning `dict[str, float]` not `NeuromodulatorLevels`, because conflict penalty is negative and `NeuromodulatorLevels` is [0,1]-bounded); the v3 LLM prompt schema gains the 12th field `hormone_response_i_predict` (9-key dict) and the 6th hard rule "no other fields"; the R79-D framework's `inject_v3_prompt` is rewritten to render the v3 system prompt live from the R79-A `AggressiveRadicalEmbodiedPromptPath` (replacing the hard-coded v2-draft file), per-tick fed by the most recent tick's `result.stage_results` for the four `{placeholder}` slots. 34 new tests: `test_r79c_learned_param_category.py` 4 + `test_hormone_predict_corroborator_config.py` 9 + `test_hormone_predict_corroborator_classify.py` 10 + `test_hormone_predict_corroborator_aggregate.py` 5 + `test_r79c_hormone_coverage.py` 6. R79-C requirement package triple at `docs/requirements/79c-r79c-hormone-predict-corroborator-and-5ht-oxy-opioid-drivers/`. Test baseline: 900 passed (866 R79-B baseline + 34 new), 2 pre-existing perf-flakes; R21 ad-hoc logging guard green; composition owner-boundary guard green. R79-D end-to-end probe (NOOP gateway) shows 5-HT non-constant (0.028-0.045 range), Oxy non-constant (0.012 range), Opioid near-constant (0.003 range — the salience aggregator's `uncertainty=1.0` default zeros the `(1-uncertainty)` multiplier; remediation is R80 composition scope). HEAD-era: R79-C.)
