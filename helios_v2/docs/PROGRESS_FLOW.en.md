...
> Last synced: R92 (Wall-Clock Real Timestamps — adds the new infrastructure owner `helios_v2.wall_clock`: a single-method `WallClock` protocol + `SystemWallClock` (lazy `time.time()`) + the deterministic test `FixedWallClock` + the reserved metadata key `RECEIVED_AT_WALL_METADATA_KEY`; a pure-fact source with no cognitive policy and no cognitive-owner imports. Three additive consumption points: `RuntimeFrame.tick_wall_seconds` is seeded once per tick by the kernel; `CliChannelDriver.submit_line` stamps `received_at_wall` into `InboundPacket.metadata` at arrival time (not drain time, preserved verbatim by `02` onto `Stimulus.metadata`); `PersistedExperienceRecord.created_at_wall` is written by both record bridges (SQLite migrates an existing file in place via PRAGMA-guarded `ALTER TABLE`; pre-R92 rows read back as `None`). The R91 `_present_field_summary_text` gains an additive `last input: <X.X>s ago` clause (rendered alongside `pacing: <signal>`; NTP-rewind clamped to `0.0s`; either side absent => clause omitted). `RuntimeProfile.wall_clock` is the opt-in capability seam; `assemble_production_runtime` defaults a `SystemWallClock` ON, `assemble_runtime` keeps `wall_clock=None` (legacy byte-for-byte). **Owner count +1 (infra `helios_v2.wall_clock`, blue); the cognitive stage chain, the cognitive owner boundaries, and existing owners' maturities are unchanged.** Test baseline: ≥ 1059 passed / 4 skipped (network-free). Prior sync R90 (P5 evaluation framework complete — all tests-only, read-only diagnostics under `tests/`, no owner/stage-chain/boundary change).
