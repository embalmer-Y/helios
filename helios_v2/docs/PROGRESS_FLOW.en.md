...
> Last synced: R79-B (Channel-Catalog Runtime Injection and LLM Channel Arbitration — `AggressiveRadicalPromptProfile` capability bundle (frozen dataclass, fail-fast `__post_init__` on empty / duplicate / non-string channels); `RuntimeProfile.aggressive_radical_prompt_profile` field with `assemble_runtime` integration (bootstrap id switch to `v3-aggressive-radical` + path selection to `AggressiveRadicalEmbodiedPromptPath`); `ready_channels: tuple[str, ...] = ()` class field on `FirstVersionEmbodiedPromptRequestBridge` and `SemanticEmbodiedPromptRequestBridge` with `_resolved_channels` projection (falls back to `("cli",)` shim for v1 default); `AggressiveRadicalChannelArbitrationPostProcessor` owner-neutral glue parses the v3 LLM JSON envelope's `i_will_send_it` / `i_send_through` / `i_want_to_say` triple and dispatches a single `OutboundPacket` via `ChannelSubsystem.dispatch_outbound` (5 fail-soft reasons: `parse_error` / `not_sending` / `channel_not_ready` / `empty_text` / `no_subsystem`; `AggressiveRadicalChannelArbitrationOutcome` frozen dataclass). 19 new tests: 13 in `tests/test_aggressive_radical_channel_arbitration.py` and 6 in `tests/test_r79b_runtime_integration.py`. R79-B requirement package triple at `docs/requirements/79b-r79b-channel-catalog-runtime-injection-and-llm-arbitration/`. Default assembly + v1 path byte-for-byte unchanged; v3 path is opt-in via `AggressiveRadicalPromptProfile(ready_channels=(...))`. Test baseline: 866 passed (847 baseline + 13 arbitration + 6 integration), 0 regression; R21 ad-hoc logging guard green; composition owner-boundary guard green. HEAD-era: R79-B.)
