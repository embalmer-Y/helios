...
| --- | --- | --- | --- | --- | --- | --- |
| `03-07` appraisal, neuromodulation, feeling, memory-affect, workspace | salience shaping, state modulation, working competition, bounded reportable content preparation | functional analogy | `A_stronger_grounding` | baseline owner set exists; with `35`/`39`/`40`/`41` the `03` appraisal owner is now fully de-shimmed — all five salience dimensions plus the aggregate judgment are real under the semantic-memory assembly (novelty = distance from stored memory; uncertainty = retrieval ambiguity, `B_functional_inspiration`; social = transport-provenance fact; threat/reward = cosine to hand-authored prototype phrase sets, `C_engineering_hypothesis` placeholder anchor; aggregate = bounded convex combination of the five, with first-version placeholder weights); with `36` the `04` neuromodulator levels derive from real appraisal salience, and with `37`+`38` both of `04`'s downstream consumers are real, so the `03 → 04 → 05`/`09` chain is now real end to end on every channel. `06-07`, `04`'s dual-timescale dynamics, and the cortisol/inhibition hard gate are still shim | the threat/reward grounding is the weakest link: a hand-authored, English-centric prototype anchor (`C_engineering_hypothesis`), explicitly a placeholder to be replaced by P5 learning, `06` memory-affect grounding from past outcome valence, or a slow `11`-LLM re-appraisal — it must not be over-claimed, and the aggregate inherits this weakness for its threat/reward contribution; the aggregate weights are likewise a first-version placeholder allocation (P5-learnable); `06-07` remain shim; `04`/`05` are real and both now carry dual-timescale cross-tick dynamics and resume across restart (R43/R44); the real `05` feeling does not yet measurably shape `06` memory-affect/behavior; so modulation shapes later owners more narrowly than a strong biological analog would imply | `03`, `04`, `05`, `06`, `07`, `08`, `09`, `34`, `35`, `36`, `37`, `38`, `39`, `40`, `41`, `43`, `44` |
