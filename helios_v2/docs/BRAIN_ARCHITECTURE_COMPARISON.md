...
| `gap_persistence_and_learning` | Until `33` the runtime had no durable memory at all: every continuity packet, autobiographical trace, and the whole experience stream were lost on process exit, so the system could not subjectively re-enter a prior existence and no learning could accumulate. With `33` (P2 opener) the `15` experience-writeback continuity stream is durably persisted (SQLite file backend) and, on an opt-in persistent assembly, re-enters the `10` directed-retrieval thought window after a restart; with `34` recall is now semantic rather than recency-only: a backend-neutral embedding owner embeds each record at write, the store ranks by bounded cosine similarity, and `10` recalls the experience most relevant to the current query (including a prior session's, after a restart). With `42` the genuinely cross-tick *runtime-continuity state* now also survives a restart: a durable latest-state checkpoint owner persists the `09` continuation-pressure state and the `18`/`24` long-horizon continuity (deferred records + threads) after each tick and restores them at startup, so a restarted runtime resumes its prior continuation pressure and continuity threads instead of starting inert (it no longer recalls only *what happened* while forgetting *where its thinking was*). The gap remains partially open: the `15` stream, the `09`/`18`/`24` cross-tick continuity, and (with `43`) the `04` neuromodulator levels are durable, while `06` memory items, `05` feeling, and identity state `14` are not yet durable (`05`/`14` are not even cross-tick in-process state today, so they join the `42` checkpoint only once their dual-timescale/persisted carry lands), `03` novelty does not yet consume the embedding substrate beyond the first P3 slice, and no parameter learning yet consumes the durable base (that is P5). With `43` the `04` neuromodulator state additionally evolves across ticks (a dual-timescale leaky-integrator carrying prior-tick levels: phasic fast toward the appraisal-derived drive, tonic slow back to baseline) and resumes across a restart through the same checkpoint (snapshot v2), so affect now has real cross-tick momentum rather than recomputing from baseline each tick | Brain-like claims about a continuous self require that experience survive and re-enter across sessions by relevance, and that the system's in-flight subjective and affective state (what it was attending to, the tendencies it keeps returning to, how aroused/stressed it is) also resume across sessions; semantic restart recall of real experience, cross-restart resumption of cross-tick continuity, and now cross-tick + cross-restart affect dynamics all exist, but richer persistence (`05`/`14`/`06`), state-family breadth, and actual learning remain future P2/P3/P5 work | `33`, `34`, `42`, `43`, `15`, `10`, `09`, `18`, `04`, `25` |
