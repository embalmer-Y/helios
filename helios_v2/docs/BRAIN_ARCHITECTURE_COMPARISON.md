...
| `gap_interoceptive_signal_source` | The `05` feeling owner is built to consume real internal body/interoceptive signals: the `InteroceptiveFeelingRuntimeStage` filters the `02` sensory batch for `modality in {body, interoceptive}` and forwards them to `05` as `internal_signals`. But nothing currently PRODUCES such signals — the assembled sensory sources (`FirstVersionSensorySource`, the CLI driver) emit only `text` modality, so `internal_signals` is always empty, and R38's `NeuromodulatorDerivedFeelingConstructionPath` additionally ignores them this slice (feeling is derived from the `04` neuromodulator state alone). The `BODY` node in the progress-flow maps is therefore an interface-only placeholder with no owner that generates it. In a brain, interoception is a real afferent stream (cardiac, respiratory, visceral, fatigue, pain) and is bidirectional with neuromodulatory state; Helios has no physical body, so a real source must be an explicit future owner: either a simulated body-state model, or a first-version proxy that maps real compute/runtime pressure (CPU, memory, latency, error rate) into bounded interoceptive signals. No owner is assigned yet | Brain-like affect and a felt body-state require a real interoceptive afferent stream, not only top-down neuromodulator-derived feeling; until a producer exists, `05` feeling is one-directional (from `04` only) and the "felt body" is not grounded in any real internal condition | `05`, `02`, `04` |
