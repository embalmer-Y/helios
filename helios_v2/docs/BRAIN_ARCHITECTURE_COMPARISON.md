...
| --- | --- | --- | --- | --- | --- | --- |
| `03-07` appraisal, neuromodulation, feeling, memory-affect, workspace | salience shaping, state modulation, working competition, bounded reportable content preparation | functional analogy | `A_stronger_grounding` | baseline owner set exists; with `35`/`39`/`40` all five `03` salience dimensions are now real (novelty = distance from stored memory; uncertainty = retrieval ambiguity, `B_functional_inspiration`; social = transport-provenance fact; threat/reward = cosine to hand-authored prototype phrase sets, `C_engineering_hypothesis` placeholder anchor); with `36` the `04` neuromodulator levels derive from real appraisal salience, and with `37`+`38` both of `04`'s downstream consumers are real, so the `03 → 04 → 05`/`09` chain is now real end to end on every channel (reward→dopamine, threat→cortisol, novelty/uncertainty→norepinephrine; `04`→`05` feeling and `04` NE→`09` gating). `06-07`, `04`'s dual-timescale dynamics, and the cortisol/inhibition hard gate are still shim | the threat/reward grounding is the weakest link: a hand-authored, English-centric prototype anchor (`C_engineering_hypothesis`), explicitly a placeholder to be replaced by P5 learning, `06` memory-affect grounding from past outcome valence, or a slow `11`-LLM re-appraisal — it must not be over-claimed as real threat/reward understanding; `06-07` remain shim; `04`/`05` are real but stateless (no prior-tick tonic/phasic decay or feeling persistence); the real `05` feeling does not yet measurably shape `06` memory-affect/behavior; the constant aggregate-salience estimator is now the next sensible de-shim; so modulation shapes later owners more narrowly than a strong biological analog would imply | `03`, `04`, `05`, `06`, `07`, `08`, `09`, `34`, `35`, `36`, `37`, `38`, `39`, `40` |
