...
| --- | --- | --- | --- | --- | --- | --- |
| `03-07` appraisal, neuromodulation, feeling, memory-affect, workspace | salience shaping, state modulation, working competition, bounded reportable content preparation | functional analogy | `A_stronger_grounding` | baseline owner set exists; with `35` the `03` novelty dimension is the first real signal (novelty = distance from stored memory via the `34` embedding substrate, the first cognitive consumer of it), with `36` the `04` neuromodulator levels are the first real downstream consumer of that signal (dopamine/norepinephrine/cortisol derived from real appraisal salience), and with `37` the real `04` norepinephrine level is the first `04` signal to shape a downstream cognitive decision: under the semantic-memory assembly it is coupled into the `09` gate decision through an owner-owned bounded arousal term, so elevated neuromodulatory arousal measurably raises fire propensity. The other four `03` dimensions, `04`'s dual-timescale dynamics, `05-07`, and the `05`/cortisol-gate couplings are still shim, and cross-domain integration depth is still uneven | the four non-novelty `03` dimensions and `05-07` remain constant first-version shim; `04` is now appraisal-derived but stateless (no prior-tick tonic/phasic decay), and only its norepinephrine→`09` arousal coupling is real (the `05` feeling layer and the cortisol/inhibition hard gate are not yet coupled), so modulation shapes later owners more narrowly than a strong biological analog would imply | `03`, `04`, `05`, `06`, `07`, `08`, `09`, `34`, `35`, `36`, `37` |
