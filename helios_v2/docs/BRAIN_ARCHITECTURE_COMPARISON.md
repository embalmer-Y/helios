...
| `09-11` gating, retrieval, internal thought | selective ignition, memory-guided maintenance, internally generated cognitive continuation | functional analogy | `A_stronger_grounding` | owner chain exists and runs in the runtime path; with `37` the `09` gate decision became the first real consumer of an `04` neuromodulator level (norepinephrine couples into selective ignition as a bounded arousal term), and with `48` the gate's `global_activation_level` term is grounded in the real `07` workspace activation (max retained `workspace_score_hint`), so the gate now consumes two real upstream signals | multi-tick continuity is improved but still bounded and policy-shallow compared with long-horizon subjective continuity; the gate's remaining inputs (`workload_pressure`, `temporal_signal`, `drive_urgency_signal`, `dmn_available`, stimulus projection) remain first-version shim | `09`, `10`, `11`, `18`, `37`, `46`, `48` |
